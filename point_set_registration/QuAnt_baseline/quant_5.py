...
def mat_loss_func2(A_mat, x_gt, lamb1, lamb2, network, input):
  ...
  #result = torch.flatten(result, start_dim=1) + lamb1 * network.sparse_loss(input) - lamb2 * torch.abs(result1 - res_value[:,1])
  #result = result.mean()
  result = torch.flatten(result, start_dim=1) -   (result1 - (1/(2**TOTAL_RESULT_VECTOR)) * torch.sum(res_value, dim = 1))
  #result = result.sum()
  #result = result / (BATCH_SIZE - difference.sum() + 1)
  result = result.mean()
  return result, difference.detach().sum().item()
...
class Network(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc1 = nn.Linear(21*21, 32)
        self.fc2 = nn.Linear(32, 32)
        self.fc3 = nn.Linear(32 + 21*21, 32)
...
def ana(df, lr):
  ...
  np.save('pure_5_' + str(lr), histogram)
