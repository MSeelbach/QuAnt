...
def mat_loss_func3(A_mat, x_gt, lamb1, lamb2, network, input):
  ...
  A_mat[torch.abs(A_mat) < 0.001] = 0
  sparse_loss = torch.where(torch.abs(A_mat) > 0, torch.abs(A_mat - 1), A_mat)
  #unique = torch.sum(A_mat, dim=1)
  #unique = torch.sum(unique, dim=1)
  #unique *= -0.5
  #unique += (1/(2**TOTAL_RESULT_VECTOR)) * res_value[:,0]
  #result = torch.flatten(result, start_dim=1) ) #+ lamb2 * unique
  #result = result.mean()
...
def mat_loss_func2(A_mat, x_gt, lamb1, lamb2, network, input):
  ...
  #result = torch.flatten(result, start_dim=1) + lamb1 * network.sparse_loss(input) - lamb2 * torch.abs(result1 - res_value[:,1])
  #result = result.mean()
  result = torch.flatten(result, start_dim=1) + lamb1 * network.sparse_loss(input) -  (result1 - (1/(2**TOTAL_RESULT_VECTOR)) * torch.sum(res_value, dim = 1))
  #result = result.sum()
  #result = result / (BATCH_SIZE - difference.sum() + 1)
  result = result.mean()
  return result, difference.detach().sum().item()
...
class Network(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc2 = nn.Linear(21*21, 32)
        self.fc4 = nn.Linear(32, 32)
        self.fc5 = nn.Linear(32, 81)
        #self.fc6 = nn.Linear(256, 256)
        #self.fc12 = nn.Linear(256, 81)
    ...
    def forward(self,x):
       ...
       # x = self.fc12(x)
        ...
        x = torch.reshape(x, (reshape_size, 9,9))
    ...
    def sparse_loss(self,x):
        ...
        #x = x + x_trans
        #x = torch.sin(x)
        #loss = torch.mean(np.pi/4 - torch.abs(x)) ** 2
        return loss
...
for epoch in range(EPOCHS):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(data_train, 0):
        ...
        loss, diff_sum = criterion(outputs, labels,10e-4,0.001, net, inputs)
        diff_temp += diff_sum
...
def ana(df, lr):
  ...
  np.save('pure_3_' + str(lr), histogram)
  np.save('icp' + str(lr), histogram_icp)
