...
def gen_rotated_image(P):
  ...
  result = np.transpose(rot_mat @ np.transpose(P))
...
def generate_Data(number_of_rotations, disc_steps, num_image, num_last_img, img_ls, sig):
  ls_img = []
  for j in range(num_image, num_last_img):
    ...
    for _ in range(number_of_rotations):
      H = np.copy(P)
      print('...........')
      ind = np.random.choice(H.shape[0], int(H.shape[0]*sig), replace=False)
      print(H.shape)
      print(ind.shape)
      H_1 = H[ind, :]
      print(H_1.shape)
      ...
      print(H.shape)
      H = np.concatenate((H, H_1))
      print(H.shape)
      print(P.shape)
      print('-----------------')
      ret, a11, a22, a33, rot_mat = gen_rotated_image_fst_axis(H)
...
#df_data_test_0 = generate_Data(100, 5, 301, 331, myFiles,0.0)
...
#df_data_test_6 = generate_Data(100, 5, 301, 331, myFiles,0.25)

#df_data_test_0.to_pickle('test_dataframe_0.pkl')
#df_data_test_1.to_pickle('test_dataframe_1.pkl')
#df_data_test_2.to_pickle('test_dataframe_2.pkl')
#df_data_test_3.to_pickle('test_dataframe_3.pkl')
#df_data_test_4.to_pickle('test_dataframe_4.pkl')
#df_data_test_5.to_pickle('test_dataframe_5.pkl')
#df_data_test_6.to_pickle('test_dataframe_6.pkl')


#df_data_test_1 = pd.read_pickle('test_dataframe_1.pkl')
#df_data_test_2 = pd.read_pickle('test_dataframe_2.pkl')
#df_data_test_3 = pd.read_pickle('test_dataframe_3.pkl')
#df_data_test_4 = pd.read_pickle('test_dataframe_4.pkl')
#df_data_test_5 = pd.read_pickle('test_dataframe_5.pkl')


#data_test = DataLoader(dataset = Dataset_train(df2), batch_size = BATCH_SIZE, shuffle =False)
os.chdir(r'') # current working dict

df_data_test_1 = generate_Data(100, 5, 301, 331, myFiles,0.01)os.chdir(r'/home/mkrahn/windows/working_stuff/last_experiments_different_models_3/mixed_model_pure/3_1/')
...
class Network(nn.Module):
    ...
    def sparse_loss(self,x):
        ...
        x = torch.reshape(x, (reshape_size,TOTAL_RESULT_VECTOR))
...
'''
net1 = torch.load('9layer_diag_fist_network3d_done.pt')
net1.eval()
net2 = torch.load('9layer_diag_second_network3d_done.pt')
net2.eval()
net3 = torch.load('9layer_diag_third_network3d_done.pt')
net3.eval()
...
def analysis(df, sig):
  ...
  with torch.no_grad():
    ...
    for index, row in df.iterrows():
        ...
        x_min = torch.round((  net1(new_input) ))
        ind_ls = []
        for i in range(x_min.shape[0]):
          row = x_min[i,:]
          ind = np.where(np.all((possible_x_trans==row).cpu().numpy(),axis=1))
          ind_ls.append(ind)
        res_angle1 = np.array(ind_ls)
        ...
        res_angle1 = torch.from_numpy(res_angle1)
        ...
        x_min = torch.round((  net2(new_output) ))
        ind_ls = []
        for i in range(x_min.shape[0]):
          row = x_min[i,:]
          ind = np.where(np.all((possible_x_trans==row).cpu().numpy(),axis=1))
          ind_ls.append(ind)
        res_angle2 = np.array(ind_ls)
        ...
        res_angle2 = torch.from_numpy(res_angle2)
        ...
        mats_2 = calc_rot_mat_from_angle(0,counter_rot_2.item(),0)
        ...
        x_min = torch.round((  net3(new_output) ))
        ind_ls = []
        for i in range(x_min.shape[0]):
          row = x_min[i,:]
          ind = np.where(np.all((possible_x_trans==row).cpu().numpy(),axis=1))
          ind_ls.append(ind)
        res_angle3 = np.array(ind_ls)
        ...
        res_angle3 = torch.from_numpy(res_angle3)
        ...
        Q = calc_rot_mat_from_angle(res_angle1[0],res_angle2[0],res_angle3[0])
        Q = torch.reshape(Q,(3,3))
