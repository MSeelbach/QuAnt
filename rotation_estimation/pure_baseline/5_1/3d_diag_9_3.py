...
df = generate_Data_third(1000, 5, 1,300, myFiles)
...
class Network(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc12 = nn.Linear(32, TOTAL_RESULT_VECTOR)
...
def mat_los_func3(A_mat, x_gt, lamb1, lamb2, network, input):
  ...
  A_sum_diag = torch.diagonal(A_mat, dim1= -2, dim2=-1)
  A_sum = -0.5 * torch.sum(A_sum_diag,dim=-1)
...
for epoch in range(EPOCHS):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(data_train_third, 0):
        ...
        loss= criterion( outputs,labels) + 10e-3 * net3.sparse_loss(inputs)
        loss.backward()
        optm3.step()
        gc.collect()
...
torch.save(net3, 'pure_third_3_1.pt')
