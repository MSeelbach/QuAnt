...
df = generate_Data_fst(1000, 5, 1,500, myFiles)
...
class Network(nn.Module):
    ...
    def forward(self,x):
        input_x = x.clone()
        input_x = torch.repeat_interleave(input_x, 3, dim = 1)
        x = F.relu(self.fc2(x))
        x = F.relu(self.fc4(x))
...
for epoch in range(EPOCHS):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(data_train_fst, 0):
        ...
        loss = criterion(outputs, labels, 10e-3, 0.001, net1, inputs)
        loss.backward()
        optm1.step()
        gc.collect()
    ...
    #if epoch % 10 == 0:
    #    torch.save(net1, 'first_network3d' + str(epoch) +'.pt')
    
    
torch.save(net1, 'our_first_5_1.pt')
