...
def bruteForceAngle_flexible(mat, x_gt, possible_x_trans, possible_x): 
    ...
    results = torch.matmul(results, possible_x)
    results = torch.diagonal(results, dim1 = 1, dim2 = 2)
    results = torch.topk(results,3,largest=False,dim=1)
    ...
    a1, a2, a3 = torch.split(re_angle, [5,5,5], dim=1)
    '''
    temp = possible_x_trans[0,result_angle[0,2]]
...
possible_x_trans = torch.reshape(possible_x_trans, (1, possible_x_trans.shape[0], possible_x_trans.shape[1]))
possible_x = torch.transpose(possible_x_trans,1,2)
#possible_x = torch.transpose(possible_x_trans,0,1)
def analysis(df, sig):
  net1 = torch.load('diag_first_3_1.pt')
  net1.eval()
  net2 = torch.load('diag_second_3_1.pt')
  net2.eval()
  net3 = torch.load('diag_third_3_1.pt')
  net3.eval()
  ...
  with torch.no_grad():
    ...
    for index, row in df.iterrows():
        ...
        outputs2 = torch.diag(outputs2[0])
        ...
        outputs3 = torch.diag(outputs3[0])
